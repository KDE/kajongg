...
class RegTest(unittest.TestCase):
    ...
    def testTrueColorGame(self):
        self.scoreTest(r'b1b1b1B1 B2B3B4B5B6B7B8B8B2B2B2 fe fs fn fw Mwe LB2B2B3B4', Score(limits=1))
    def testOnlyConcealedMelds(self):
        self.scoreTest(r'B1B1B1B1B2B3B4B5B6B7B8B9DrDr fe ys Mwew LDrDrDr', Score(44, 2), rules=[20])
        self.scoreTest(r'b1B1B1b1 B2B3B4 B5B6B7 B8B8B8 DrDr fe ys Mwew LDrDrDr', Score(72, 2), rules=[20])

    def testLimitHands(self):
        self.scoreTest(r'c1c1c1 c9c9 b9b9b9b9 s1s1s1 s9s9s9 Mee Lc1c1c1c1', Score(limits=1), rules=[20])
        self.scoreTest(r'c1c1c1c1 drdr wewewewe c3c3c3C3 s1S1S1s1 Mee Lc1c1c1c1c1', Score(limits=1), rules=[20])
        self.scoreTest(r'drdr c1c1c1c1 wewewewe c3c3c3C3 s1S1S1s1 Mee Lc1c1c1c1c1', Score(limits=1), rules=[20])
        self.scoreTest(r'c1c1c1c1 wewewewe c3c3c3C3 s1S1S1s1 drdr Mee Lc1c1c1c1c1', Score(limits=1), rules=[20])
        self.scoreTest(r'b2b2b2b2 DgDgDg b6b6b6 b4b4b4 b8b8 Mee Lb2b2b2b2b2', Score(limits=1), rules=[20])
    ...
    def testAllHonours(self):
        ...
        self.scoreTest(r'wewewe wswsws WnWnWn wwwwwwww DrDr Mned LDrDrDr', Score(limits=1))
        self.scoreTest(r'wewewe wswsws WnWnWn wwwwwwww DrDr Mned LDrDrDr', Score(limits=1))
    ...
    def testRest(self):
        ...
        self.scoreTest(r'b3B3B3b3 DbDbDb DrDrDr wewewewe s2s2 Mee Ls2s2s2', Score(72, 6), rules=[20])
        ...
        self.scoreTest(r'b3b3b3b3 DbDbDb drdrdr weWeWewe s2s2 Mee Ls2s2s2', Score(76, 5),  rules=[20])
        self.scoreTest(r's2s2s2 s2s3s4 B1B1B1B1 c9C9C9c9 mes', Score(42))
        self.scoreTest(r's2s2s2 DgDg DbDbDb b2b2b2b2 DrDrDr Meew Ls2s2s2s2', Score(50, 4),  rules=[20])
        self.scoreTest(r's2s2 DgDgDg DbDbDb b2b2b2b2 DrDrDr Meew Ls2s2s2', Score(limits=1),  rules=[20])
        self.scoreTest(r's2s2 DgDgDg DbDbDb b2b2b2b2 DrDrDr mee', Score(32, 6))
        self.scoreTest(r's1s1s1s1 s2s2s2 s3s3s3 s4s4s4 s5s5 Msww Ls3s3s3s3', Score(44, 4),  rules=[20])
        self.scoreTest(r'B2C1B2C1B2C1WeWeS4WeS4WeS6S5 mee', Score(20, 3))
        self.scoreTest(r'c1c1c1 c3c4c5 c6c7c8 c9c9c9 c2c2 Meew Lc1c1c1c1', Score(limits=1),  rules=[20])
        self.scoreTest(r'b6b6b6 B1B1B2B2B3B3B7S7C7B8 mnn', Score(2))
        self.scoreTest(r'B1B1B1B1B2B3B4B5B6B7B8B9DrDr fe fs fn fw Mwew LDrDrDr', Score(52, 3),  rules=[20])
        self.scoreTest(r'B1B1B1B1B2B3B4B5B6B7B8B9DrDr fe fs fn fw Mwee LDrDrDr', Score(50, 4),  rules=[21])
        self.scoreTest(r'B1B1B1B1B2B3B4B5B6B7B8B9DrDr fe fs fn fw Mwez LDrDrDr', Score(50, 4),  rules=[22])
        self.scoreTest(r'B1B1B1B1B2B3B4B5B6B7B8B9DrDr fe fs fn fw MweZ LDrDrDr', Score(50, 4),  rules=[23])
        self.scoreTest(r'B1B1B1B1B2B3B4B5B6B7B8B8B2B2 fe fs fn fw mwe', Score(28, 1))
        self.scoreTest(r's1s2s3 s1s2s3 B6B6B7B7B8B8 B5B5 fn yn Mneka LB5B5B5', Score(32, 3),  rules=[20, 24, 25])
        self.scoreTest(r'wewe wswsws WnWnWn wwwwwwww b1b1b1 Mnez Lb1b1b1b1', Score(54, 6),  rules=[22])
    def testTerminals(self):
        # must disallow chows:
        self.scoreTest(r'b1b1 c1c2c3 c1c2c3 c1c2c3 c1c2c3 Mes Lb1b1b1', Score(26, 1),  rules = [20] )
